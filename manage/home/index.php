...
if (isset($_SESSION['form_message'])) {
    if ($_SESSION['form_message']['type'] === 'success') {
        $success_message = $_SESSION['form_message']['text'];
    } else {
        $errors[] = $_SESSION['form_message']['text'];
    }
    unset($_SESSION['form_message']);
}

if (isset($_GET['tab'])) {
    $get_tab = trim($_GET['tab']);
    if (in_array($get_tab, ['hero', 'buttons', 'about', 'contact'])) {
        $active_tab = $get_tab;
    }
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['active_tab'])) {
        $posted_tab = trim($_POST['active_tab']);
        if (in_array($posted_tab, ['hero', 'buttons', 'about', 'contact'])) {
    ...
    try {
        ...
        $pdo->commit();
        $_SESSION['form_message'] = ['type' => 'success', 'text' => $settings_data['button_update_success']];

    } catch (Exception $e) {
        $pdo->rollBack();
        $_SESSION['form_message'] = ['type' => 'error', 'text' => $settings_data['button_update_fail'] . " " . $e->getMessage()];
    }

    header("Location: " . MANAGE_HOME_URL . "?tab=" . urlencode($active_tab));
    exit();
}
...
<main class="pt-32 pb-20">
    <section id="manage-home" data-aos="fade-up">
        <div class="container mx-auto px-6 max-w-4xl">
            ...
            <div class="bg-gray-800/50 p-8 rounded-lg shadow-lg" x-data="{ tab: '<?php echo htmlspecialchars($active_tab); ?>' }">
