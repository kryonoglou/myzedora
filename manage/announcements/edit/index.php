...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        try {
            $stmt = $pdo->prepare("UPDATE announcements SET subject = ?, message = ? WHERE id = ?");
            if ($stmt->execute([$subject, $message, $announcement_id])) {
...
$page_title = $settings_data['edit_announcement_title'] . " - " . $settings_data['seo_title'];

require_once HEADER;
?>

<main class="pt-32 pb-20">
    <section id="edit-announcement" data-aos="fade-up">
        <div class="container mx-auto px-6 max-w-3xl">
            <h1 class="text-4xl font-bold text-center mb-10 section-title"><?php echo htmlspecialchars($settings_data['edit_announcement_title']); ?></h1>
            ...
            <div class="bg-gray-800/50 p-8 rounded-lg shadow-lg">
                ...
                <form action="<?php echo EDIT_ANNOUNCEMENT_URL_BASE . $announcement_id; ?>" method="POST">
                    <div class="mb-4">
                        ...
                        <input type="text" id="subject" name="subject" class="w-full bg-gray-700 border border-gray-600 rounded-lg py-2 px-4 text-white focus:outline-none focus:ring-2 focus:ring-sky-500" value="<?php echo htmlspecialchars($announcement['subject']); ?>" required>
                    </div>
                    ...
                    <div class="mb-6">
                        ...
                        <textarea id="message" name="message" rows="10" class="w-full bg-gray-700 border border-gray-600 rounded-lg py-2 px-4 text-white focus:outline-none focus:ring-2 focus:ring-sky-500" required><?php echo htmlspecialchars($announcement['message']); ?></textarea>
